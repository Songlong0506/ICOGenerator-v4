...
| `Agent` | Một "nhân sự AI". Mang `RoleKey` (vai trò), model, và tập tool được phép dùng. Instruction (system prompt) được nạp từ file `Prompts/{RoleKey}/instruction.md` theo `RoleKey`, không còn lưu trong DB. | `Domain/Agent.cs`, `Services/Agents/AgentInstructionProvider.cs` |
...
RequirementsController          (Controllers)   nhận message từ user
  └► *UseCase / *Query          (Application)    điều phối
       └► BAChatService / ProductBriefDraftService / RequirementDocsService  (Services/Requirements)
...
| Prompt theo bước | Theo vai: `Prompts/Developer/{poc-preview,implementation}.v1.md`, `Prompts/TechLead/{architecture-design,code-review}.v1.md`, `Prompts/Tester/testing.v1.md` | `{{input}}` = nội dung theo `InputSource`. `implementation` = sinh **code đa file** trong `04_Implementation/src/`; `code-review` = Tech Lead soát code đó, ghi `04_Implementation/code-review.md`. |
...
- **Tài liệu cho user vs cho dev (tách vai)**: "Write Requirement" ở trang `Requirements` **chỉ sinh Product Brief** (`ProductBrief.docx`, ngôn ngữ đời thường cho user) ở dạng draft — để user có thể chỉnh đi chỉnh lại mà không đốt token sinh lại bản kỹ thuật. **AI Design Spec** (`AIDesignSpec.docx`, bản kỹ thuật để dựng POC) chỉ được sinh **khi user bấm Approve**: `ApproveRequirementUseCase` promote Product Brief lên `V{n}` rồi gọi đồng bộ `RequirementDocsService.GenerateAiDesignSpecAsync` (lời gọi LLM từ Product Brief đã duyệt), ghi thẳng vào `02_Design/V{n}` (đã duyệt), rồi mới `StartDeliveryWorkflowAsync` dựng POC. Bộ tài liệu kỹ thuật nặng **BRD/SRS/FSD/UserStories** KHÔNG sinh ở đây — chúng là **bước 2 của Delivery Pipeline** (`AgentTaskType.TechnicalDocs`, sau POC): sau khi duyệt POC thì BA sinh chúng từ Product Brief + AI Design Spec đã duyệt rồi dừng ở cổng duyệt như mọi bước. Khác với các bước khác (agent + prompt chung), bước này chạy qua `RequirementDocsService.GenerateTechnicalDocsAsync` (worker xử lý nhánh riêng, không dùng `MaxSteps`). Prompt: `Prompts/BusinessAnalyst/product-brief.v3.md`, `Prompts/BusinessAnalyst/ai-design-spec.v1.md` và `Prompts/BusinessAnalyst/technical-docs.v1.md`.
...
| Prompt | `Prompts/Tester/testing.v1.md`, `Prompts/Developer/bugfix.v1.md` | testing yêu cầu chốt verdict; bugfix giao Developer sửa đúng chỗ theo báo cáo. |
...
- **Prompt**: `WorkflowTaskPromptBuilder` render template bước như thường rồi **nối thêm** khối `Prompts/Shared/revision.v1.md` (bàn giao lần trước + nhận xét + quy tắc "SỬA trên cái đã có, KHÔNG làm lại từ đầu"). Bước `TechnicalDocs` đi đường riêng: `GenerateTechnicalDocsAsync(revisionFeedback:)` → khối "Reviewer change request" trong `RequirementPromptBuilder.BuildTechnicalDocs` (prompt này vốn dạng "update" bộ tài liệu hiện có).
...
| Prompt | `Prompts/Shared/revision.v1.md`, `WorkflowTaskPromptBuilder`, `RequirementPromptBuilder.RevisionSection` | khối chỉnh sửa nối sau prompt gốc / chèn vào prompt TechnicalDocs. |
...
- **Một người — nhiều role** (bảng nối `AppUserRole`, thay cột `AppUser.Role` cũ): quyền của các vai trò **giao nhau** chứ không lồng nhau, nên quyền hiệu lực là **HỢP** quyền của mọi vai trò họ giữ, không phải quyền của riêng vai trò "cao nhất". Cụ thể: `IdentityServerSettings.MapRoles` giữ **mọi** role claim khớp; `SsoUserProvisioner` đồng bộ nguyên tập vai trò mỗi lần đăng nhập (IdP là nguồn sự thật — vai trò bị thu hồi bên IS4 sẽ mất ở app, claim rỗng thì **giữ nguyên** để mapping thiếu không vô tình hạ quyền); mỗi vai trò phát **một claim `ClaimTypes.Role`**; `PermissionService.HasPermissionAsync` duyệt hết các claim, có một vai trò được cấp là đủ.
