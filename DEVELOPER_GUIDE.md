...
### 7.6. Bản triển khai thực tế — file & cách hoạt động

Bản hiện thực bám sát blueprint trên. Bản đồ file:

| Thành phần | File | Vai trò |
|---|---|---|
| Giai đoạn mới | `Domain/Enums/WorkflowStageKey.cs` | thêm `ArchitectureDesign`, `Testing` (enum lưu dạng int → **không cần migration**). |
| Khai báo pipeline | `Services/Workflows/DeliveryPipeline.cs` | danh sách `Steps` (Tech Lead → Dev → Tester) + `First` + `Next(stage)`. Thêm vai = thêm một dòng ở đây. |
| Prompt theo bước | `Prompts/Workflow/{architecture-design,implementation,testing}.v1.md` | user-prompt cho từng `AgentTaskType`, chỗ `{{input}}` = nội dung hand-off. |
| Dựng prompt | `Services/Workflows/WorkflowTaskPromptBuilder.cs` | map `AgentTaskType` → template + chèn input. Đăng ký DI trong `AddWorkflowServices()`. |
| Khởi tạo | `Services/Workflows/WorkflowOrchestrator.cs` | tạo `WorkflowRun` + task ở `DeliveryPipeline.First` (không còn cứng Dev). |
| Hand-off | `Services/Workflows/AgentTaskWorker.cs` | sau khi task xong: `DeliveryPipeline.Next(...)` → enqueue task vai kế (Input = output) hoặc kết thúc workflow. |

Lưu ý thiết kế:
- **Hành vi sâu theo vai** (Tech Lead phân tích kiến trúc, Tester viết test…) đến từ *system-prompt* của agent — file instruction theo `RoleKey` ở `Prompts/Agents/Instructions/`. Template ở `Prompts/Workflow/` chỉ mô tả *việc của bước*. Worker vẫn generic.
- **POC template** (`poc-template.html`) chỉ được copy vào workspace ở bước `Implementation`; các bước khác bỏ qua.
- **Luồng requirement-draft** (`AgentTaskType.RequirementAnalysis`) là một workflow một-bước riêng, *không* đi qua pipeline này — worker vẫn xử lý nó tách bạch ở nhánh đầu.
- **Bước 6 (điểm dừng cho con người)** vẫn để ngỏ: hiện hand-off chạy thẳng một mạch. Muốn chèn duyệt giữa các bước, cho nhánh hand-off set `WaitingForHuman` thay vì enqueue ngay, rồi thêm `ApproveStageUseCase` (xem mục 7.4 bước 6).
