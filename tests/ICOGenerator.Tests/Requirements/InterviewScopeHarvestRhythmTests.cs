...
public class InterviewScopeHarvestRhythmTests
{
    ...
    {
        Assert.False(InterviewScopeService.ShouldHarvest(EarlyInTheInterview, null, 0, turns));
    }
    ...
    {
        Assert.True(InterviewScopeService.ShouldHarvest(ReadyForTheScreenTable, null, 0, 22));
    }
    ...
    {
        Assert.True(InterviewScopeService.ShouldHarvest(ReadyForTheScreenTable, null, 22, 23));
    }
    ...
    public void WaitsForABatch_AfterTheTableWasConfirmed()
    {
        for (var newTurns = 1; newTurns < InterviewScopeService.HarvestBatchThreshold; newTurns++)
        {
            Assert.False(InterviewScopeService.ShouldHarvest(
                ReadyForTheScreenTable, ConfirmedScreens, 30, 30 + newTurns));
        }

        Assert.True(InterviewScopeService.ShouldHarvest(
            ReadyForTheScreenTable, ConfirmedScreens,
    ...
    public void StaysQuiet_AfterConfirmation_WhenTheMainFlowGroupSlippedBack()
    {
        ...
        Assert.False(InterviewScopeService.ShouldHarvest(
            slipped, ConfirmedScreens, 30, 30 + InterviewScopeService.HarvestBatchThreshold));
    }
    ...
    {
        Assert.False(InterviewScopeService.ShouldHarvest(ReadyForTheScreenTable, null, 22, 22));
        Assert.False(InterviewScopeService.ShouldHarvest(ReadyForTheScreenTable, ConfirmedScreens, 30, 30));
    }
    ...
    {
        Assert.False(InterviewScopeService.ShouldHarvest(null, null, 0, 5));
    }
