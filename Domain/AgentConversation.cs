...
public class AgentConversation
{
    ...
    // JSON array (chuỗi) NĂM BẢNG CHỐT còn lại của buổi phỏng vấn, mỗi bảng ở đúng lượt BA bày nó ra:
    // FlowMap (FlowMapRow[] — luồng nghiệp vụ theo vai trò), ScreenScopeMap (ScreenScopeRow[] — màn hình
    // dự kiến), EntityMap (EntityMapRow[] — đối tượng nghiệp vụ + vòng đời), ReportMap (ReportMapRow[] —
    // báo cáo / thống kê), NotificationMap (NotificationMapRow[] — sự kiện × người nhận). null với mọi
    // lượt khác.
    // Năm cột riêng chứ không một cột chung: cùng khuôn với ColumnMap/PermissionMatrix, và một cột chung
    ...
    // transcript không biết BA đã đề xuất gì để người dùng gật/lắc.
    // Là nội dung yêu cầu nên mã hóa at rest như Message/Suggestions.
