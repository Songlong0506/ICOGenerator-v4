...
- **Bước 1** — `ProposeRemainingGapsUseCase` → `GapProposalService`: MỘT lời gọi LLM (`BusinessAnalyst/gap-proposals.v1.md`) soạn một phương án mặc định cho MỖI nhóm còn trống, bám hội thoại + "Điều đã chốt" + "Sẽ xây gì". Mục nào model trả về mà không khớp một nhóm ĐANG trống đều bị loại (cổng chỉ lấp ô trống, không mở đường cho model thêm chủ đề). Structured output là tùy chọn từng model và **mặc định TẮT**, nên service có nhánh parse JSON từ text — thiếu nó thì cổng không chạy được trên chính cấu hình mặc định.
- **Bước 2** — `ConfirmRemainingGapsUseCase`: mỗi dòng người dùng để nguyên hoặc gõ đè, rồi chốt một lần. Điều đã chốt được ghi vào **hội thoại** như lời của chính họ (không phải một cột riêng — mọi tầng chắt lọc đều ăn theo transcript, một đường ghi riêng sẽ bị lượt distill sau viết đè), bản đồ bao phủ được gộp **ngay trong lượt này** (chờ tới lượt chat sau thì cổng vừa bấm trông như không có tác dụng), và lượt BA đóng lượt chỉ được MỜI bấm "Write Requirement" khi `RequirementReadinessGate` pass trên bản đồ vừa gộp — cùng bất biến với đường chat, nên nút và panel không thể vênh nhau.
...
- **Thêm endpoint theo project**: action mới nhận `projectId` (hoặc id tài nguyên con) phải gọi `IProjectAccessGuard` trước khi làm gì khác — xem các action hiện có trong `RequirementsController` làm mẫu.
