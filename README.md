...
**Cổng "chốt nhanh phần còn lại"** (`POST /Requirements/ProposeGaps` → `ConfirmGaps`): đường TĂNG TỐC cho phần phỏng vấn còn lại. Prompt ép "mỗi lượt một câu hỏi" và cổng readiness chỉ mở khi MỌI nhóm áp dụng đã `[RÕ]` — hai điều đúng về chất lượng nhưng cộng lại thành hàng chục lượt chat, và người dùng nghiệp vụ bận thì bỏ dở chứ không có cách nào rút ngắn. Panel "Tiến độ khai thác" vì vậy có thêm nút **"⚡ Chốt nhanh N nhóm còn lại"** (hiện khi bản đồ còn dòng trống):

- **Bước 1** — `ProposeRemainingGapsUseCase` → `GapProposalService`: MỘT lời gọi LLM (`BusinessAnalyst/gap-proposals.v2.md`) soạn một phương án cho MỖI nhóm còn trống, bám hội thoại + "Điều đã chốt" + "Sẽ xây gì". Mục nào model trả về mà không khớp một nhóm ĐANG trống đều bị loại (cổng chỉ lấp ô trống, không mở đường cho model thêm chủ đề). Structured output là tùy chọn từng model và **mặc định TẮT**, nên service có nhánh parse JSON từ text — thiếu nó thì cổng không chạy được trên chính cấu hình mặc định.
- **Bước 2** — `ConfirmRemainingGapsUseCase`: mỗi nhóm hiện như một **câu hỏi chọn-một** — ba gợi ý đồng hạng (`proposal` là gợi ý đầu, hai gợi ý còn lại từ `options`), ô **"Ý khác"** để tự nhập, nút **"Để sau"** nếu chưa muốn trả lời — rồi chốt một lần. Bấm một gợi ý là chốt luôn dòng đó: không có nút "đồng ý" riêng, vì bấm gợi ý đã vừa là chọn nội dung vừa là đồng ý. Điều đã chốt được ghi vào **hội thoại** như lời của chính họ (không phải một cột riêng — mọi tầng chắt lọc đều ăn theo transcript, một đường ghi riêng sẽ bị lượt distill sau viết đè), bản đồ bao phủ được gộp **ngay trong lượt này** (chờ tới lượt chat sau thì cổng vừa bấm trông như không có tác dụng), và lượt BA đóng lượt chỉ được MỜI bấm "Write Requirement" khi `RequirementReadinessGate` pass trên bản đồ vừa gộp — cùng bất biến với đường chat, nên nút và panel không thể vênh nhau.

**"Suy ra" phải kiếm được, không phải khai được.** Bản đầu của cổng chọn sẵn "Đúng ý" cho cả danh sách, nên bấm ngay sau câu trả lời đầu tiên là nhận 11 phương án phần lớn do model tự nghĩ ra — đã tick sẵn — và người dùng phải gõ đè gần như từng dòng: chậm hơn hẳn trả lời từng câu trong chat. Nay mỗi phương án phải tự khai `confidence` + `basis`, và `GapProposalService` **chấm lại lời khai đó bằng luật tất định**: căn cứ phải đủ dài, không được chép lại chính phương án, và ≥70% số chữ phải THẬT SỰ có trong hội thoại/"Điều đã chốt"/"Sẽ xây gì" — không qua được thì hạ về phỏng đoán, bất kể model viết gì. UI chỉ chọn sẵn phần suy ra (gợi ý đầu sáng lên, kèm dòng "Căn cứ:" để soi được), phần phỏng đoán để ở "Để sau" và **không** được gửi đi nếu người dùng không động vào — ranh giới này nói bằng HÀNH VI của dòng chứ không dán nhãn từng dòng (badge "BA suy ra từ trao đổi"/"BA phỏng đoán" và nhãn "BA đề xuất" đã bỏ: chúng lặp lại điều màn hình đã nói, riêng "BA đề xuất" gắn theo vị trí nên hiện cả trên dòng BA chỉ đoán), chỉ đoạn dẫn đầu khung nêu lại bằng con số — nhóm đó ở nguyên trạng thái trống, BA hỏi tiếp trong chat. Ô "Ý khác" mở ra thì **mồi sẵn nội dung đang chọn**: phần lớn lượt tự nhập là chỉnh vài chữ của một gợi ý, bắt gõ lại cả câu chính là thao tác làm bản đầu của cổng chậm hơn chat. Lệch chuẩn luôn nghiêng về "phỏng đoán": một phương án đúng bị gắn nhãn phỏng đoán chỉ tốn một cú bấm, còn một phương án bịa đội lốt "suy ra" thì được chọn sẵn và đi thẳng vào hội thoại như lời của chính người dùng.

Nguyên tắc "bước soạn tài liệu KHÔNG được tự giả định" giữ nguyên: phương án chỉ thành yêu cầu sau khi người dùng bấm chốt — y hệt một câu "Đồng ý" bấm trên chip gợi ý, chỉ khác là chốt nhiều điểm một lần.
...
| `BusinessAnalyst/requirement-coverage.v2.md` | Cập nhật bản đồ bao phủ yêu cầu — kiêm "giám khảo" của cổng "Write Requirement" (ready suy tất định từ bản đồ, không có prompt readiness riêng) |
