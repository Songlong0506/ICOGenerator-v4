...
- **Bước 2** — `ConfirmRemainingGapsUseCase`: mỗi dòng người dùng để nguyên, bấm một lựa chọn khác, gõ đè hoặc **để sau**, rồi chốt một lần. Điều đã chốt được ghi vào **hội thoại** như lời của chính họ (không phải một cột riêng — mọi tầng chắt lọc đều ăn theo transcript, một đường ghi riêng sẽ bị lượt distill sau viết đè), bản đồ bao phủ được gộp **ngay trong lượt này** (chờ tới lượt chat sau thì cổng vừa bấm trông như không có tác dụng), và lượt BA đóng lượt chỉ được MỜI bấm "Write Requirement" khi `RequirementReadinessGate` pass trên bản đồ vừa gộp — cùng bất biến với đường chat, nên nút và panel không thể vênh nhau.

**"Suy ra" phải kiếm được, không phải khai được.** Bản đầu của cổng chọn sẵn "Đúng ý" cho cả danh sách, nên bấm ngay sau câu trả lời đầu tiên là nhận 11 phương án phần lớn do model tự nghĩ ra — đã tick sẵn — và người dùng phải gõ đè gần như từng dòng: chậm hơn hẳn trả lời từng câu trong chat. Nay mỗi phương án phải tự khai `confidence` + `basis`, và `GapProposalService` **chấm lại lời khai đó bằng luật tất định**: căn cứ phải đủ dài, không được chép lại chính phương án, và ≥70% số chữ phải THẬT SỰ có trong hội thoại/"Điều đã chốt"/"Sẽ xây gì" — không qua được thì hạ về phỏng đoán, bất kể model viết gì. UI chỉ chọn sẵn phần suy ra (kèm hiện dòng "Căn cứ:" để soi được), phần phỏng đoán để ở "Để sau" và **không** được gửi đi nếu người dùng không động vào — nhóm đó ở nguyên trạng thái trống, BA hỏi tiếp trong chat. Mỗi dòng còn có 2–3 **chip lựa chọn** để đổi ý bằng một cú bấm thay vì gõ lại cả câu. Lệch chuẩn luôn nghiêng về "phỏng đoán": một phương án đúng bị gắn nhãn phỏng đoán chỉ tốn một cú bấm, còn một phương án bịa đội lốt "suy ra" thì được chọn sẵn và đi thẳng vào hội thoại như lời của chính người dùng.
