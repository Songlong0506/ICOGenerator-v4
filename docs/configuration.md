...
| `Llm:Proxy:UseDefaultCredentials` | false | Trả lời `407` bằng tài khoản Windows đang chạy app (Negotiate/NTLM) ⇒ trỏ **thẳng** `Address` vào proxy công ty, không cần relay px/CNTLM. Chỉ có tác dụng trên Windows; chạy như service thì tài khoản service phải có quyền qua proxy |
| `Llm:Proxy:BypassList` | `[]` | Host đi thẳng, không qua proxy: host đầy đủ (`llm.corp.local`) hoặc hậu tố tên miền (`.bosch.com`) — **không phải regex** (`LlmProxy` tự escape). Tương đương `no_proxy`; thiếu nó thì bật proxy là giết mọi model tự host nội bộ |
| `Poc:RuntimeCheck:{Enabled,BrowserPath,AutoInstall,AutoInstallTimeoutSeconds}` | true / trống / true / 300 | Tầng chạy POC trong Chromium headless ([getting-started.md](getting-started.md#chromium-cho-tầng-kiểm-poc-và-webpilot)). `BrowserPath` trống ⇒ dùng bộ Playwright của máy, chưa có thì tự tải một lần (`AutoInstall`). Fail-open toàn phần |
| `WebPilot:{Enabled,Headless,UserDataDir,BrowserPath,AutoInstall,NavigationTimeoutSeconds,MaxTextChars,MaxControls,MaxSteps}` | true / false / trống / trống / true / 30 / 3000 / 40 / 20 | Vai agent lái trình duyệt thật ([agents-and-tools.md](agents-and-tools.md#webpilot--vai-lái-trình-duyệt-thật)). Dùng chung cơ chế tìm/tải Chromium với `Poc:RuntimeCheck` qua `PlaywrightLauncher`. `Headless=false` là mặc định CÓ CHỦ Ý: người dùng nhìn được agent lái, và đăng nhập tay một lần vào trang nội bộ — phiên nằm trong `UserDataDir` (trống ⇒ `{AgentWorkspace:RootPath}/.webpilot-profile`) nên các lượt sau agent vào thẳng. Máy chủ Linux không có display thì đặt `Headless=true` |
