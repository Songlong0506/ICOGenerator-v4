...
**Tầng thứ ba: bản đồ bao phủ làm CHỨNG NHÂN cho nhật ký (`DecisionUnderHarvestGuard`).** Hai tầng trên lo dòng nhật ký *viết sai*; tầng này lo dòng *không được viết ra*. Nhật ký bỏ sót là hỏng **im lặng** tuyệt đối: không mặt UI nào bày nó ra nữa, nên không ai bắt được. Ca thật ghi ngay trong `decision-log.v1.md` (dự án *JD Libary 5*): sau 26 lượt — người dùng đã chốt vai trò nào gán JD, bộ trường của một JD, bộ trường của một lần gán, việc bỏ ngày hết hạn, việc không cần báo cáo và quy mô sử dụng — nhật ký chỉ có **ĐÚNG MỘT dòng**. Thiệt hại không nằm ở nhật ký mà ở tầng sau: `RequirementConflictService` soát mâu thuẫn **bằng chính danh sách này**, nên một nhật ký gần rỗng làm cả cơ chế soát mâu thuẫn **mù**, và mâu thuẫn của buổi đó đi thẳng vào tài liệu.

Prompt đã có "Phép thử chống BỎ SÓT", nhưng nó bắt model tự chấm chính mình — lưới yếu nhất trong mọi lưới ở đây, và nó đã rách đúng ca trên. Phép thử nay chạy **bằng code**, và thứ chạy được nó là việc bản đồ bao phủ *là một bộ đọc ĐỘC LẬP đọc đúng lô lượt đó*: mọi dòng `[RÕ]`/`[MỘT PHẦN]` của bản đồ đều bắt buộc kèm khối `{nguồn: …}` trích ngắn lời người dùng. Vậy nên **một trích dẫn của bản đồ nằm trong lời người dùng của lô ⇒ lô này CÓ nội dung nghiệp vụ**, đã được một bên thứ hai xác nhận; nhật ký không dài thêm và không sửa dòng nào trong khi đó ⇒ nghi bỏ sót. Đây chính là lý do hai bộ chắt phải ở **tách** nhau: gộp `BARequirementCoverage` với `BADecisionLog` làm một lời gọi thì không còn ai đối chiếu với ai, và ca JD Libary 5 lại trở về vô hình.

Guard **chỉ nghi, không sửa** — nó không bịa được dòng còn thiếu. Nghi ⇒ chắt lại **ĐÚNG MỘT lần**, chở theo chính các trích dẫn đã khớp làm chỉ dẫn (chỉ thẳng vào chỗ vừa bỏ qua, không phải một lời "cố lên"), và **không nới** luật chống suy diễn. Lần chắt lại chỉ được nhận khi nó THẬT SỰ đổi nhật ký; không đổi hoặc lỗi thì giữ kết quả lần đầu và dời con trỏ như thường — nghi ngờ không được phép thành vòng lặp. Ba tính chất đi kèm: khớp trích dẫn bỏ qua dấu câu/khoảng trắng/hoa thường (model trích gần đúng chứ hiếm khi đúng từng ký tự) và có **ngưỡng độ dài tối thiểu** để các mẩu như *"có"* không thành căn cứ; lời gọi thêm mang purpose **riêng** `BADecisionLogRecheck` để đo được tần suất guard nổ trên màn AI Call Logs; và vì cả hai lời gọi đều nằm trên đường **hậu kỳ** (sau frame done) nên người dùng không chờ thêm giây nào. Chốt bằng `DecisionUnderHarvestGuardTests` + `DecisionLogServiceTests`.
