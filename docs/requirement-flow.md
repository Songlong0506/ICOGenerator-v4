...
- **Thống kê đứng TRƯỚC các dòng mẫu.** `SourceContextBuilder.Truncate` cắt text mỗi nguồn ở `Llm:SourceUpload:MaxTextCharsPerFile` (mặc định 20.000) và **giữ phần đầu**; một sheet rộng (tới 40 cột, ô tới 200 ký tự) ăn hết ngân sách đó chỉ bằng 29 dòng mẫu, còn bảng nhiều sheet thì các sheet sau mất sạch. Đặt thống kê xuống dưới là để thứ đắt nhất rơi ra trước — mất danh mục cột không suy lại được, mất vài dòng mẫu thì không sao.
- Phần bảng mẫu tự nói rõ nó chỉ là dòng đầu và không được dùng để suy ra danh mục.

Chốt bằng `SpreadsheetTextExtractorTests` + `SourceAckReadbackRuleTests`, và chấm điểm bằng hai scenario `source-ack` trong golden set.
