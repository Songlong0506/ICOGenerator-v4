...
- **Ngữ cảnh: chính TRANSCRIPT.** Thứ phân biệt được "hỏi tiếp phần còn thiếu" với "hỏi lại điều vừa được trả lời" là danh sách câu hỏi THẬT — bản đồ chỉ có độ phân giải theo nhóm nên không làm được việc đó. Danh sách ấy nằm sẵn trong các lượt assistant của cửa sổ nguyên văn: `BAChatService.BuildAssistantContext` dựng lại mỗi lượt cũ thành đúng JSON `{message, suggestions, multiSelect, openEnded, questions, ready}`, tức chở cả câu của lượt hỏi một câu lẫn mảng `questions` của lượt gộp. `requirement-chat.v4.md` vì thế bắt BA **đọc lại các lượt của chính nó** trước khi viết câu hỏi.
  ...
  - **Transcript không chỉ dạy FORMAT, nó dạy NHỊP — nên trường nào có trong schema trả lời thì phải có ở đây.** Mỗi trường bỏ sót là một chiều thông tin model không còn phân biệt được giữa các lượt cũ của chính nó, và bộ ví dụ ấy mạnh hơn mọi lệnh trong prompt vì nó là ví dụ **đã được chấp nhận**. Ca thật đã đo (log BAChat 2026-09-06, dự án quản lý khóa học bắt buộc): `openEnded` không được echo, nên lượt **MỞ hợp lệ** (không chip vì đang xin lời kể) và lượt **ĐÓNG quên chip** quay lại ngữ cảnh dưới cùng một mặt chữ `"suggestions": []`. Hai lượt mở hợp lệ ở đầu buổi là đủ: 26 lượt tiếp theo là câu hỏi ĐÓNG không một chip nào, các phương án bị viết thành văn xuôi ngay trong câu hỏi (*"… hay là chỉ cần thông tin cơ bản ạ?"* — đúng dạng mà mục *"Ví dụ về `message`"* của prompt cấm bằng tên), và cả buổi phỏng vấn chỉ có đúng một hàng nút để bấm. **Không chốt chặn nào xóa chip cả** — AI Call Logs ghi `suggestions: []` ngay ở response thô: model chỉ chép lại hình dạng của chính nó. Vì cờ này phải sống qua một lần lưu mới lên được transcript, nó là một **cột** (`AgentConversation.OpenEnded`) chứ không phải thứ suy lại từ nội dung lượt; cột đó cũng là thứ giữ ô nhập ở chế độ "mời kể" sau khi F5. Đối xứng ở phía prompt: `requirement-chat.v4.md`, mục *"BA HÌNH DẠNG ĐÓ LÀ TẤT CẢ"* — `suggestions: []` chỉ hợp lệ khi `openEnded: true` hoặc `questions` không rỗng.
