...
4. Cần nhiều tình huống trên **cùng một điểm xuất phát** (cùng buổi phỏng vấn BA, cùng file nguồn, cùng các bảng đã chốt) thì nhân bản dự án ở trang Projects thay vì seed tay: nút nhân bản mỗi dòng, chọn "bản sao đầy đủ" (rẽ nhánh từ đúng chặng đang đứng) hoặc "chỉ phần yêu cầu" (chạy lại delivery từ đầu). Bản sao không mang theo log gọi model, link chia sẻ POC và task đang dở — xem [workspace-and-poc.md](workspace-and-poc.md#nhân-bản-dự-án-thử-nhiều-tình-huống-trên-cùng-một-điểm-xuất-phát).
5. Lái UI bằng Playwright; selector cổng duyệt: `#delivery-gate`, `#dg-approve-form`, `#dg-revise-btn`, `#revise-modal`... Gate poll ~2.5s, worker nhặt task ~2s.
