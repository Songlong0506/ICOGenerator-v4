...
| **Projects** (trang chủ) | `Projects` | `Index` (lọc theo chủ nếu không có `ProjectsViewAll`; cột `Status` hiện chặng của dự án — badge dùng chung `Shared/_ProjectStatusBadge`, xem [overview.md](overview.md#chặng-của-dự-án-projectstatus)), `POST Create`/`Update`/`Clone` (nhân bản dự án), `Mockup` (xem POC sandbox; `review=True` tiêm annotator), `PocReview` (review POC + ghim ghi chú), `GET PocComments`, `POST AddPocComment`/`WithdrawPocComment`/`ReopenPocComment`, `GET PocShareLinks`/`SearchAssociates`, `POST CreatePocShareLink`/`RevokePocShareLink`, `POST TriagePocFeedback` (phân loại ghi chú cho hộp xác nhận), `POST DispatchPocFeedback` (gửi Dev chỉnh demo / gửi BA sửa tài liệu theo đúng tập con), `POST AcceptPoc`/`WithdrawPocAcceptance` (nghiệm thu bản demo và rút nghiệm thu — nghiệm thu KHOÁ chat BA + ghi chú POC, xem [workspace-and-poc.md](workspace-and-poc.md#nghiệm-thu-là-công-tắc-hai-chiều-và-nó-khoá-nội-dung)), `DownloadSource` (zip) | `ProjectsView`; Create + Clone: `ProjectsCreate`; thêm ghi chú POC: `ProjectsView` (như Feedback — quyền View đủ để gửi phản hồi của mình); thu hồi (không xoá cứng, chỉ ghi chú còn `Open`): chủ ghi chú hoặc `DeliveryAdvance`; nghiệm thu/rút nghiệm thu: `RequirementsManage` |
| **Requirements** (workspace chat BA) | `Requirements` | `Index`, `POST ChatStream` (SSE — đường chat chính, stream token), `POST Chat` (fallback postback), `GET ChatReplyStatus`, `POST UploadSource`/`DeleteSource`, `GET SourceContent`, `POST WriteRequirement`, `POST ConfirmColumnMap` (bảng cột của file bảng tính), `POST ConfirmPermissionMatrix` (bảng phân quyền, chốt ở cuối buổi phỏng vấn), `POST ConfirmFlowMap`/`ConfirmScreenScope`/`ConfirmEntityMap`/`ConfirmReportMap`/`ConfirmNotificationMap` (năm bảng chốt còn lại — luồng, màn hình, đối tượng nghiệp vụ, báo cáo/thống kê, thông báo/nhắc nhở), `POST ReviseBrief`, `POST Approve`, `POST ConfirmAssumptions`/`ReviseAssumptions` (cổng giả định), `POST NewChat`, `POST RetryWorkflow`, `GET WorkflowStatus`/`WorkflowStream` (SSE), `GET DocumentRevisions`/`DocumentRevisionDiff`/`DocumentPreview`/`DownloadDocument` | `RequirementsView`; mọi thao tác ghi: `RequirementsManage`. `ChatStream`/`NewChat`/`UploadSource`/`ReviseBrief` còn bị khoá khi bản demo đã được nghiệm thu — xem [workspace-and-poc.md](workspace-and-poc.md#nghiệm-thu-là-công-tắc-hai-chiều-và-nó-khoá-nội-dung). Gói rà soát ("Download Context") đã chuyển sang Agent Dashboard — xem dòng dưới. |
...
| **Agent Dashboard** (điều phối delivery) | `AgentDashboard` | `Index`, `GET WorkflowStatus`/`ActiveAgents`/`AgentStats`/`AgentActivity`/`AgentCallLogs`/`CallLogDetail`/`CallLogImage`/`CallLogExport`/`CallLogTurnExport`/`DocumentPreview`, `GET DownloadReviewPackage` (xuất cả chuỗi hội thoại → Brief → Spec → POC ra `.zip` để nhờ AI khác rà soát), `POST ApproveStage`/`RejectStage`/`RequestRevision`/`RetryWorkflow`/`UpdateDeliveryConfig` | `AgentsView`; các POST cổng duyệt: `DeliveryAdvance`. `DownloadReviewPackage` cần thêm `RequirementsDownloadPackage` (nút "Download Context" trên command bar cũng ẩn khi thiếu quyền này) và **co gói theo quyền**: bỏ POC khi thiếu `ProjectsView` (`AgentsView` đã là cổng của cả controller nên AI Design Spec luôn có) — xem [requirement-flow.md](requirement-flow.md#tải-trọn-gói-để-nhờ-một-ai-khác-rà-soát) |
| **Agents** (cấu hình agent) | `Agents` | `Index`, `Checklist(role)` (bật/tắt bài học của MỘT vai — nút nằm trong panel của vai đang chọn, chỉ hiện với vai có đường học: xem `LearnedChecklistRoles`), `POST Update` (model, temperature, tools...), `POST SaveChecklist(role, ...)` | `AgentsView` / `AgentsManage`. Nhóm tool khai `[ToolGroupAllOrNothing]` (hiện chỉ `WebTools`) hiện thành MỘT dòng và được cấp cả gói — `UpdateAgentUseCase` nở lựa chọn ra cả nhóm nên chốt nằm ở server, không phải ở JavaScript ([agents-and-tools.md](agents-and-tools.md#nhóm-tool-cấp-cả-gói)) |
| **WebPilot** (thử nghiệm agent lái browser) | `WebPilot` | `Index`, `POST Run` (SSE — chạy lượt và stream tiến độ + ảnh chụp màn hình) | `AgentsView` / `AgentsManage`. **Dùng lại quyền nhóm Agents, cố ý không khai quyền mới**: `SeedRolePermissionsAsync` chỉ seed khi bảng `RolePermissions` còn trống, nên một `AppPermission` mới sẽ không tới tay role nào trên DB đang chạy — trang mở ra là trang chết với mọi role trừ SuperAdmin. Không action nào nhận `projectId` (lượt chạy luôn gắn vào dự án hộp cát riêng), nên không có gì để `[RequireProjectAccess]` kiểm |
...
- Quyền mức hành động: enum `AppPermission` (29 quyền — xem cột "Quyền" ở bảng trên). `PermissionCatalog`
