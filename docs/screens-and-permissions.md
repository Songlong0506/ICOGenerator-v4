...
| **Agents** (cấu hình agent) | `Agents` | `Index`, `Checklist(role)` (bật/tắt bài học của MỘT vai — nút nằm trong panel của vai đang chọn, chỉ hiện với vai có đường học: xem `LearnedChecklistRoles`), `POST Update` (model, temperature, tools...), `POST SaveChecklist(role, ...)` | `AgentsView` / `AgentsManage` |
