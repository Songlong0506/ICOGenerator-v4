...
| `BusinessAnalyst/interview-outlook.v3.md` | "Triển vọng phỏng vấn": `workedExamples` — ảnh chụp trạng thái các ví dụ tính thử người dùng đã xác nhận, chắt ở HẬU KỲ mỗi lượt chat vì nó chỉ được tiêu thụ ở bước sinh AI Design Spec. `openQuestions` **không còn ở đây**: danh sách câu hỏi ra đời cùng bản đồ bao phủ trong `requirement-coverage.v5.md` — xem [requirement-flow.md](requirement-flow.md#một-lượt-distill-hai-cột-bản-đồ-trạng-thái-và-danh-sách-câu-hỏi) |
| `BusinessAnalyst/interview-scope.v1.md` | `scopeAdditions` (DELTA phạm vi màn hình, ghép thẳng vào bảng màn hình ở trạng thái chờ duyệt). Tách khỏi file trên vì nó chạy theo một nhịp khác hẳn — im lặng tới khi bản đồ ngã ngũ VÀ ba bảng đứng trước đã hết việc, rồi gộp bù trọn quãng, sau đó theo lô (`InterviewScopeService.ShouldHarvest`); giữ luật đặt tên màn hình và luật "chỉ màn hình, chức năng thì gộp vào màn chứa nó" (`InterviewScopePromptSplitTests`) — xem [requirement-flow.md](requirement-flow.md#nhịp-của-lượt-chắt-lọc-phạm-vi-màn-hình) |
