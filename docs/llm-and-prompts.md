...
| File | Dùng cho |
|---|---|
| `BusinessAnalyst/requirement-chat.v4.md` | Lượt chat BA. HAI nhóm bị prompt CẤM hỏi bằng câu hỏi, cả hai được chốt bằng bảng ở cuối buổi tại đúng lượt cổng tất định của nó mở: «Phân quyền theo nghiệp vụ» (trường `permissionMatrix`, `PermissionMatrixGate` — xem [requirement-flow.md](requirement-flow.md#bảng-phân-quyền-chốt-nhóm-phân-quyền-ở-cuối-buổi)) và «Thông báo / nhắc nhở» (trường `notificationMap`, `NotificationMapGate` — xem [requirement-flow.md](requirement-flow.md#bảng-thông-báo-bảng-cuối-cùng)). Nhóm «Báo cáo / thống kê» thì NGƯỢC LẠI: vẫn hỏi bằng câu hỏi, và bảng của nó (trường `reportMap`, `ReportMapGate`) chỉ được bày ra SAU khi nhóm đã `[RÕ]` — xem [requirement-flow.md](requirement-flow.md#bảng-báo-cáo-mỗi-báo-cáo-là-một-màn-hình). Prompt còn phải NÓI ra ba chốt chặn tất định mà code áp lên chính lượt của nó, nếu không model viết lại đúng hình dạng bị chặn ở mọi lượt (`InterviewGuardPromptRuleTests`): câu nhóm «Luồng ngoại lệ» phải đứng một mình và cả nó lẫn nhóm «Báo cáo» đều bị **xóa bộ chip có/không** (`InterviewQuestionRules`); lượt user nhắc tới một file mà dự án chưa có tài liệu nguồn nào thì lượt trả lời **bị thay bằng lời xin file** (`SourceRequestTurn`); và `openEnded: true` KHÔNG cứu được một lượt không hỏi gì khỏi chốt chặn lượt câm. Prompt còn phải dạy cờ `summaryCheck` — cờ model tự khai cho nhịp tóm tắt kiểm chứng, thứ DUY NHẤT cho code biết lượt nào được bù bộ hai chip xác nhận (`SummaryCheckFlagRuleTests`); code không đoán theo chữ nữa. **Đặc tả trường của sáu bảng KHÔNG còn ở đây** — file này chỉ giữ bất biến *"không có khối `## LƯỢT NÀY: BÀY BẢNG …` trong ngữ cảnh ⇒ không dựng bảng nào"*; sáu file `table-*.v1.md` ngay dưới mang phần còn lại. **Lệnh cấm hỏi lẻ của hai nhóm chốt-bằng-bảng cũng không còn ở đây**: nó là một nhánh trạng thái của cổng nên sống ở `BAChatPromptBlocks.PermissionMatrixDeferred` / `NotificationDeferred` và chỉ vào ngữ cảnh ở đúng lượt còn hiệu lực; file này giữ con trỏ tới khối đó cùng bất biến thứ hai — *không có khối cấm ⇒ hỏi nhóm ấy như mọi nhóm khác*, đường thoát của dự án không có vòng đời trạng thái nào (`InterviewTablePromptTests`, [requirement-flow.md](requirement-flow.md#lệnh-để-cuối-đừng-hỏi-lẻ-ở-lại-code-và-vì-sao-nó-không-đi-cùng-đường)) |
| `BusinessAnalyst/source-ack.v3.md` | Lượt MỞ tài liệu nguồn (docx/xlsx/PDF/ảnh) ngay sau upload; kiêm ghi `sourceNotes` cho các hình — lượt DUY NHẤT model nhìn thấy ảnh. Hai hình dạng, do `BASourceAckPrompt.TurnShape` chọn tất định: **bảng tính chưa chốt cột** ⇒ chỉ trả `columns` (**bảng cột**: mỗi cột một dòng, ý nghĩa ĐIỀN SẴN, tích sẵn cột nghiệp vụ) kèm `message` ngắn giới thiệu file — CẤM kể lại chi tiết và CẤM cụm "Chỗ chưa chắc"; **Word/PDF/ảnh** ⇒ bản đọc lại đầy đủ + câu hỏi đóng như cũ. Luật đọc bảng chốt bằng `SourceAckReadbackRuleTests`: nghĩa của cột lấy từ khối `#### Thống kê cột` chứ không suy từ dòng mẫu, và **hai loại cột của hệ cũ** (hạ tầng + dẫn xuất như `Days Rem`) phải bỏ tích — xem [requirement-flow.md](requirement-flow.md#bảng-cột-chốt-phạm-vi-cột-của-file-bảng-tính) |
...
| `BusinessAnalyst/table-flow-map.v1.md`, `table-entity-map.v1.md`, `table-report-map.v1.md`, `table-screen-scope.v1.md`, `table-permission-matrix.v1.md`, `table-notification-map.v1.md` | Sáu khối `## LƯỢT NÀY: BÀY BẢNG …` — nửa LUẬT (đặc tả từng trường của một bảng) của lượt bày bảng, cùng dạng và cùng lý do tồn tại với `source-readback.v1.md`. `BAChatService.AppendTableBlocks` nạp **đúng một** file, ở đúng lượt `InterviewTableGate.Select` mở cổng của bảng đó; nửa DỮ LIỆU (phạm vi màn hình, đối tượng đã chốt, dòng gieo, danh sách người nhận) vẫn dựng ở `BAChatPromptBlocks`. `table-screen-scope` là file duy nhất có **hai lời mở đầu** loại trừ nhau (bày đầu / bày lại) dùng chung một bộ luật, tách bằng dòng đánh dấu `# shape:first` / `# shape:reshow` / `# rules` và đọc bằng `BAChatPromptBlocks.Section` (fail-open: mất dòng đánh dấu ⇒ cả template thành phần luật). Chốt bằng `InterviewTablePromptTests` — trong đó có phép so contract: prompt chỉ được dạy điền `evidence` cho bảng mà contract THẬT SỰ có trường ấy (`entityMap`, `permissionMatrix`, `notificationMap`), ba bảng còn lại phải nói thẳng là không có |
| `BusinessAnalyst/interview-scope.v1.md` | `scopeAdditions` (DELTA phạm vi màn hình, ghép thẳng vào bảng màn hình ở trạng thái chờ duyệt). **Lượt chắt lọc hậu kỳ DUY NHẤT còn lại** — nó tách khỏi lượt "triển vọng phỏng vấn" cũ (`interview-outlook.v3.md`, đã gỡ) vì chạy theo một nhịp khác hẳn — im lặng tới khi bản đồ ngã ngũ VÀ ba bảng đứng trước đã hết việc, rồi gộp bù trọn quãng, sau đó theo lô (`InterviewScopeService.ShouldHarvest`); giữ luật đặt tên màn hình và luật "chỉ màn hình, chức năng thì gộp vào màn chứa nó" (`InterviewScopePromptSplitTests`) — xem [requirement-flow.md](requirement-flow.md#nhịp-của-lượt-chắt-lọc-phạm-vi-màn-hình) |
| `BusinessAnalyst/screen-step-placement.v1.md` | XẾP CHỖ cho các bước luồng mà không chức năng nào trong bảng màn hình nhận phụ trách — chạy ngay trước khi bảng hiện ra, trả `placements[]` (bước → màn hình · chức năng). Thay cho dòng nhắc hỏi ngược người dùng bước đó thuộc màn nào; code vẫn là nơi bắt lỗ hổng và nơi duyệt lời xếp chỗ — xem [requirement-flow.md](requirement-flow.md#ô-phục-vụ-bước-và-lượt-xếp-chỗ-cho-bước-mồ-côi) |
| `BusinessAnalyst/product-brief.v3.md` | Sinh Product Brief (Write Requirement). Nhận thêm khối **"Trạng thái đã chắt từ hội thoại"** (`WorkedExamples` / `OpenQuestions`) làm danh sách kiểm — xem [requirement-flow.md](requirement-flow.md#chỉ-mục-của-chính-hội-thoại-đi-kèm-lượt-soạnsoátsửa-brief) — cùng **bản Brief đã duyệt gần nhất** và **tóm tắt hội thoại cũ** khi transcript đã bị cắt bớt: [Ngữ cảnh gửi lên model ở vòng soạn Brief](requirement-flow.md#ngữ-cảnh-gửi-lên-model-ở-vòng-soạn-brief) |
| `BusinessAnalyst/product-brief-review.v2.md` | Vòng tự soát Product Brief. Mở đầu bằng phép **đối chiếu máy móc** từng dòng "Ví dụ đã xác nhận" với bản nháp, rồi mới soi bằng mắt; ngoài các lỗi so với hội thoại còn bắt ba lỗi nhìn thấy được ngay trong chính bản nháp: tài liệu tự mâu thuẫn, tính năng không có màn hình/vai trò để thực hiện, và dữ liệu mồ côi (`BriefTraceabilityRuleTests`). Nội dung truy được về bản đã duyệt/tóm tắt là **hợp lệ**, không tính là "tự thêm ngoài hội thoại" |
| `BusinessAnalyst/ai-design-spec.v1.md` | Sinh AI Design Spec sau Approve — gồm mục `## 6b. Permission Matrix` chép từ bảng phân quyền người dùng đã chốt (phạm vi dữ liệu phải thành điều kiện lọc thật, không phải một câu mô tả) và mục `## 14. Acceptance Criteria` chép NGUYÊN VĂN các dòng "Hoàn thành khi: …" của Product Brief đã duyệt (`BriefAcceptanceCriteria`); `SpecBriefParityChecker` soát ba tầng màn hình/quy tắc/câu nghiệm thu và cho BA sửa một vòng nếu lệch. Mục `## 12. Assumptions` liệt kê bằng ngôn ngữ nghiệp vụ những điều spec tự quyết vì Product Brief không nói |
...
| `BusinessAnalyst/poc-ui-convention.v1.md` | Chắt lọc các ghi chú thuộc nhóm "chỉnh bản demo" đã sửa xong thành QUY ƯỚC TRÌNH BÀY dùng lại được của dự án — thứ duy nhất chở góp ý giao diện qua một vòng dựng lại POC (xem [workspace-and-poc.md](workspace-and-poc.md#góp-ý-giao-diện-sống-sót-qua-một-vòng-dựng-lại-poc-poc-ui-conventionsjson)) |
| `BusinessAnalyst/requirement-coverage.v5.md` | Cập nhật bản đồ bao phủ yêu cầu, **danh sách câu hỏi** và **ví dụ đã xác nhận** trong MỘT lời gọi (`items` + `questions` + `workedExamples`) — kiêm "giám khảo" của cổng "Write Requirement" (ready suy tất định từ bản đồ, không có prompt readiness riêng). `known` là **mảng chuỗi** — mỗi điều người dùng đã nói một phần tử, chở trạng thái MỚI NHẤT (đính chính thì xoá phần tử cũ), và gánh luôn luật chống bịa của trường `evidence` đã bỏ ([vì sao](requirement-flow.md#known-là-danh-sách-không-phải-một-ô-tóm-tắt)). Năm luật gắn với chốt chặn code: `text` của một câu hỏi phải **hỏi được một điều cụ thể** (câu rỗng nghĩa kiểu *"các quy tắc khác (nếu có)"* và câu tường thuật trạng thái bị `CoverageQuestionGuard` xoá); hai nhóm chốt-bằng-bảng **không có câu hỏi nào**; nhóm còn câu hỏi `MỞ` thì **không được `[RÕ]`** (`CoveragePendingGuard`); dòng quy tắc **chở con số** bị `CoverageWorkedExampleGuard` hạ `[MỘT PHẦN]` khi chưa có ví dụ tính thử nào (`InterviewGuardPromptRuleTests`); và một dòng còn `[RÕ]`/`[MỘT PHẦN]` mà `known` rỗng thì `CoverageKnownLossGuard` trả lại phần đã ghi nhận cũ. `workedExamples` là mảng chuỗi ĐỨNG NGANG HÀNG với hai trường kia, **không** nằm trong `known`: mỗi mục phải là một cặp đầu vào cụ thể → kết quả kỳ vọng người dùng đã xác nhận (chép một mẩu `known` sang là hình dạng sai hay gặp nhất), ví dụ bị bác thì XOÁ, và `null` ≠ `[]` — trường vắng mặt giữ nguyên cột, mảng rỗng ghi đè ([vì sao](requirement-flow.md#ví-dụ-đã-xác-nhận-về-chung-lượt-distill)) |
| `BusinessAnalyst/organization-context.v2.md` | Khung render bức tranh tổ chức |
| `BusinessAnalyst/organization-scope.v1.md` | Ranh giới phạm vi: sản phẩm chỉ phục vụ nhà máy Bosch Đồng Nai — cấm BA gợi ý phạm vi vượt nhà máy ("Toàn Bosch Việt Nam"…), cho sẵn thang phạm vi hợp lệ (orgUnit → department → toàn nhà máy, bậc cuối gọi tên là *"toàn bộ nhân viên internal của nhà máy"*). Cùng khối này chốt luôn: **SỐ người dùng không phải câu hỏi mà là hệ quả của phạm vi** — tổng số nhân sự internal và số nhân sự từng department đã nằm trong khối bối cảnh render bên dưới, nên cấm dựng bậc thang đầu người (*"Dưới 500 nhân viên"*, *"Trên 1000 nhân viên"*); vế KHỐI LƯỢNG (bao nhiêu bản ghi/khóa học, mỗi tháng thêm bao nhiêu) thì vẫn hỏi bằng con số, và không ghép chung một chip với vế người dùng (`BAChatUserScaleRuleTests`). Đính vào MỌI lời gọi BA, kể cả khi `OrgUnits` còn trống. Người dùng nói "toàn công ty"/"tất cả nhân viên Bosch" ⇒ hiểu ngầm là toàn nhà máy, ghi nhận và đi tiếp; khối này là hằng số sản phẩm nên KHÔNG được chèn vào câu "mình ghi nhận…" như lời người dùng, cũng không được làm một vế của mâu thuẫn |
| `BusinessAnalyst/organization-platform.v1.md` | Nền tảng đã chốt của nhà máy, ba ràng buộc cùng hạng. (1) **Chỉ có DUY NHẤT kênh thông báo email** — nhóm "Thông báo / nhắc nhở" chỉ hỏi *ai nhận* và *khi nào*, cấm hỏi "muốn báo qua kênh nào" và cấm gợi ý Teams/SMS/Zalo/push (`BAChatNotificationChannelRuleTests`). (2) **Chỉ đăng nhập bằng SSO qua IdentityServer** với tài khoản Bosch — cấm hỏi cách đăng nhập, kể cả câu nghe rất nghiệp vụ *"mỗi người có cần tài khoản riêng không?"*, và cấm gợi ý tài khoản nội bộ / đăng ký mới / Google / tài khoản dùng chung / tài khoản riêng cho external; vẫn phải hỏi **ai được vào app** và **vai trò gán từ đâu** (`BAChatLoginRuleTests`). SSO phủ **cả internal lẫn external** — external có tài khoản Bosch và đăng nhập y hệt, không phải ngoại lệ của đăng nhập; chỗ họ khác là **không có bản ghi trong dữ liệu HR**, nên phạm vi người dùng và nguồn vai trò của riêng nhóm đó vẫn phải hỏi. (3) **Danh sách orgUnit và nhân sự của MỌI ứng dụng trong nhà máy đồng bộ tự động từ hệ thống COMPAS** — cấm hỏi *ai quản lý/cập nhật* hai danh mục đó, cấm hỏi *chúng vào ứng dụng bằng đường nào*, cấm chip "HR"/"HRBP"/"Có người tải file lên"/"Nhập tay trong ứng dụng", cấm đưa màn hình quản lý orgUnit/nhân viên vào phạm vi (`BAChatOrgDirectoryRuleTests`). Ngoại lệ này phải có mặt ở cả mục "NGUỒN của dữ liệu" của `requirement-chat.v4.md` và ở `requirement-coverage.v5.md` (cả dòng *Dữ liệu / danh mục chính* lẫn chuẩn cắt ngang "danh mục phải có người quản lý") — BA bị cấm hỏi nên một dòng bị hạ vì chúng sẽ kẹt `[MỘT PHẦN]` vĩnh viễn. Vẫn phải hỏi thứ ứng dụng **tự gắn thêm** lên một orgUnit/một con người (JD do ai soạn/duyệt…) và nhóm **external** không có trong COMPAS. Đính vào MỌI lời gọi BA như khối ranh giới phạm vi; cả ba là hằng số sản phẩm nên KHÔNG được chèn "qua email"/"đăng nhập bằng SSO"/"đồng bộ từ COMPAS" vào câu "mình ghi nhận…" |
| `TechLead/architecture-design[-bosch].v1.md`, `TechLead/code-review.v1.md`, `Developer/poc-preview.v1.md`, `Developer/implementation[-bosch].v1.md`, `Developer/bugfix.v1.md`, `Developer/build-fix.v1.md`, `Developer/pull-request.v1.md`, `Tester/testing.v1.md` | Từng bước pipeline theo vai (`{{input}}` = nội dung theo `InputSource`); bản `-bosch` dùng khi `Project.IsUseBoschTemplate`. Bốn file thiết kế/hiện thực nhắc lại ràng buộc **thông báo chỉ có email** — pipeline KHÔNG nhận khối ngữ cảnh tổ chức, ràng buộc chỉ tới được qua tài liệu nên phải có bản của riêng chúng |
| `Shared/revision.v1.md` | Khối "Yêu cầu chỉnh sửa" nối sau prompt gốc của bước |
| `Shared/stage-revision-lesson.v1.md` | Chắt nhận xét ở cổng duyệt thành bài học cho vai đã chạy bước — xem [delivery-pipeline.md](delivery-pipeline.md#học-từ-nhận-xét-ở-cổng-duyệt) |
...
| `Shared/tool-agent-native.v1.md` | Khung prompt chung cho agent chạy tool (bọc `{{instruction}}` của vai + `{{learnedChecklist}}` — bài học vai rút được từ nhận xét của người duyệt) |
