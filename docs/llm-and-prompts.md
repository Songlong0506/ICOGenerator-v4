...
| `BusinessAnalyst/interview-outlook.v2.md` | "Triển vọng phỏng vấn": `openQuestions` / `workedExamples` — hai ảnh chụp trạng thái, chắt lại sau MỖI lượt chat vì tồn đọng câu hỏi được nạp thẳng vào ngữ cảnh lượt sau. Mỗi `openQuestions` là object `{ group, text }`: `group` chép đúng một trong 12 nhãn bản đồ bao phủ và là đầu vào của `CoveragePendingGuard` — v1 bắt model tự gõ thẻ `[Nhãn]` ở đầu chuỗi, xem [requirement-flow.md](requirement-flow.md#hình-dạng-hai-danh-sách-tồn-đọng-cũng-json-và-nhóm-là-một-trường) |
...
| `BusinessAnalyst/requirement-coverage.v4.md` | Cập nhật bản đồ bao phủ yêu cầu — kiêm "giám khảo" của cổng "Write Requirement" (ready suy tất định từ bản đồ, không có prompt readiness riêng). Hai luật gắn với chốt chặn code: mẩu `còn thiếu:` phải **hỏi được một điều cụ thể** (mẩu rỗng nghĩa kiểu *"các quy tắc khác (nếu có)"* bị `RequirementReadinessGate` bỏ qua vì cổng phát nguyên văn nó lên màn hình), và dòng quy tắc **chở con số** bị `CoverageWorkedExampleGuard` hạ `[MỘT PHẦN]` khi chưa có ví dụ tính thử nào (`InterviewGuardPromptRuleTests`) |
