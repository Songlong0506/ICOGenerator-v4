...
- **Phần markup của audit truy vấn DOM thật** (`PocDom` + AngleSharp, CSS selector), không quét chuỗi. Khung vỏ do `PocTemplate` dựng nên đúng là máy sinh, nhưng **nội dung feature bên trong là do model viết** — và đó là loại markup làm regex gãy: thuộc tính đứng trước `class` (`<div data-roles="X" class="nav-item">` không khớp mẫu chuỗi cố định), dấu `>` trong giá trị thuộc tính cắt thẻ sớm, `id="…"` nhắc trong literal JavaScript bị tính là id trùng, `data-id` bị nhận nhầm là `id`, bảng lồng bảng làm phạm vi quét bảng ngoài cụt. Khi regex gãy ở đây nó **không báo lỗi — nó báo "OK"**, đúng chế độ hỏng mà audit sinh ra để chặn. Đi kèm: bỏ bước xóa `<!-- -->`/`<style>`/`<script>` bằng regex trước khi quét (comment là node riêng, nội dung script/style là văn bản) và bộ đếm độ sâu `<div>` tự viết để tìm thẻ đóng. Cây ở đây khớp với cây Chromium mà `PocRuntimeChecker` lái, nên hai tầng kiểm cùng nhìn một thứ. `PocSpec.cs` **không** thuộc diện này — nó parse Markdown của spec, không phải HTML.
...
- **Review POC (ghim ghi chú lên phần tử)**: `GET /Projects/PocReview?projectId=` nhúng POC trong iframe ở chế độ review (`Mockup?review=True` tiêm `wwwroot/js/poc-annotator.js` lúc phục vụ — file trên đĩa không đổi). Người xem bật "chế độ ghim" (nút bật/tắt trên **command bar** — một công tắc CÓ TRẠNG THÁI, tác động lên cả trang, nên nó cần chỗ đứng cố định ở vùng phải của thanh lệnh chứ không thành một hàng nút riêng bên trên iframe), click phần tử → annotator gửi mô tả (màn hình `data-view`, nhãn, CSS selector, vị trí %) lên trang cha qua postMessage → lưu bảng `PocComments`. Pin đánh số vẽ ngay trên phần tử. Sandbox giữ nguyên (origin opaque, không cookie) — mọi thao tác ghi đều từ trang cha. Các ghi chú `Open` được gom vào "Yêu cầu chỉnh sửa" tại cổng POC (xem [delivery-pipeline.md](delivery-pipeline.md#cổng-duyệt-gates--trạng-thái-waitingforhuman)).
  ...
  - **Bố cục hai cột, mỗi cột một câu hỏi.** Cột trái (`.poc-main-col`) là **bản demo + những gì nói về chính bản demo**: khung iframe, rồi ngay dưới nó thẻ "Ghi chú trên POC" (danh sách ghi chú chung, ô nhập, nút đóng vòng "Gửi ghi chú cho đội xử lý", bảng lịch sử ghi chú). Cột phải (`.poc-comments-panel`) chỉ còn phần **đối chiếu yêu cầu**: kịch bản kiểm thử + các vòng đã dựng. Ghi chú ở cột trái vì một mục trong danh sách và một pin vẽ trên demo là cùng một thứ nhìn từ hai phía — để chúng ở hai cột là bắt mắt người dùng nhảy qua lại; và vì trước đây nó nằm SAU toàn bộ danh sách kịch bản, nên POC càng nhiều kịch bản thì đúng hai thao tác KẾT THÚC buổi review càng bị đẩy sâu. Hai hệ quả kỹ thuật: (a) `#pocReviewRoot #pocFrame` **cao hơn** trang khách (`calc(100vh - 210px)`, tối thiểu 640px) — số trừ chỉ còn đủ cho header + page-head + command bar, không trừ thêm để chừa chỗ cho thẻ ghi chú phía dưới, vì một khung demo lùn buộc người xem cuộn BÊN TRONG iframe, tức cuộn một trang nằm trong một trang; (b) click một mục ghi chú sẽ kéo khung demo về tầm mắt (`scrollIntoView`) **chỉ khi** nó đã trôi qua mép trên, nếu không cú nháy pin bên trong iframe là vô hình.
  - **Thẻ chứa khung demo không có padding, và mang hai icon "mở tab riêng" + "toàn màn hình" ở góc trên phải.** `#pocReviewRoot .poc-frame-panel` đặt `padding: 0` (trang khách `#pocShareRoot` giữ nguyên padding vì thẻ đó còn chứa `.poc-toolbar`): bản demo tự mang supergraphic + topbar + sidebar của nó, nên viền trắng của `.card` chỉ là một cái khung thứ hai lồng ngoài cái khung đã có — và nó ăn mất chiều cao của thứ duy nhất trên trang này đáng được chỗ. Hai đường tác động lên riêng bản demo rời command bar xuống thành icon nổi trong `.poc-frame-actions` neo vào **góc trên phải** khung (góc trên trái là header sidebar của chính POC — tên ứng dụng + nút gập `X`; đặt icon bấm được của trang cha sát một nút bấm được của demo là mời bấm nhầm, còn mép phải chỉ có chữ BOSCH tĩnh): khác với công tắc ghim, chúng không mang trạng thái nào của cả trang và chỉ có nghĩa với đúng một đối tượng, nên đặt chúng chạm vào thứ chúng mở/phóng thì người dùng khỏi phải đối chiếu một nhãn chữ ở tận thanh lệnh với thứ sẽ được mở.
    - **Mở tab riêng** là `<a target="_blank">` chứ không phải nút JS — giữ nguyên chuột giữa/Ctrl+click và không phụ thuộc `poc-review.js`.
    - **Toàn màn hình** (`#pocFullscreenBtn`) buộc phải là nút JS, và nó phóng **cả thẻ** `.poc-frame-panel` chứ không phóng riêng iframe: pin ghi chú và khung tô sáng lúc "chỉ chỗ" là do TRANG CHA vẽ đè lên khung demo, phóng mỗi iframe sẽ đẩy chúng xuống dưới lớp toàn màn hình — mất đúng thứ người ta đang review. Hai đường vì Fullscreen API có thể bị chính sách trình duyệt từ chối: đường thật (`requestFullscreen`, CSS `:fullscreen`) và đường dự phòng khi promise reject (`.is-fullscreen` — lớp phủ `position: fixed`, thoát bằng Esc do handler của trang). Trạng thái nút luôn đọc lại từ DOM (`document.fullscreenElement` + class) và nghe `fullscreenchange`, vì người dùng thoát bằng Esc hoặc nút của trình duyệt thì không cú click nào chạy qua handler — một biến cờ riêng sẽ lệch ngay lần đầu.
...
- **Một đường đóng vòng cho người dùng nghiệp vụ** ngay tại trang POC Review (cần `RequirementsManage`). Nút "Gửi ghi chú cho đội xử lý" chỉ hiện khi danh sách CÓ ghi chú `Open` — cả hai bước dưới đây chỉ quét đúng trạng thái đó, nên với danh sách rỗng nút ấy là lời mời bấm vào chỗ không có gì để bấm. Khối được render sẵn `hidden` rồi `renderList()` mở ra sau mỗi lượt nạp/ghim/thu hồi, chứ không quyết ở Razor: danh sách nạp bằng `fetch` sau khi trang render, và số ghi chú đổi ngay trong buổi review mà không tải lại trang. Hai bước:
...
"Share"), không còn nằm lọt trong panel ghi chú ở cột trái. Hai nút không bao giờ cùng
...
[`ProjectStatus`](overview.md#chặng-của-dự-án-projectstatus). Trang POC Review **không** in badge chặng ở
đầu trang (chỉ danh sách Projects in): trạng thái nghiệm thu đã được nói kỹ hơn ngay tại chỗ người dùng
vấp phải nó — khối "✓ Đã nghiệm thu" trong thẻ ghi chú, kèm ai chốt, lúc nào và điều gì đang khoá. Badge
chỉ nói được tên chặng, nên hai chỗ cùng nói một trạng thái thì chỗ mờ hơn là nhiễu.
