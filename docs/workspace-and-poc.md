...
- **Review POC (ghim ghi chú lên phần tử)**: `GET /Projects/PocReview?projectId=` nhúng POC trong iframe ở chế độ review (`Mockup?review=True` tiêm `wwwroot/js/poc-annotator.js` lúc phục vụ — file trên đĩa không đổi). Người xem bật "chế độ ghim" (nút bật/tắt trên **command bar** — một công tắc CÓ TRẠNG THÁI, tác động lên cả trang, nên nó cần chỗ đứng cố định ở vùng phải của thanh lệnh chứ không thành một hàng nút riêng bên trên iframe), click phần tử → annotator gửi mô tả (màn hình `data-view`, nhãn, CSS selector, vị trí %) lên trang cha qua postMessage → lưu bảng `PocComments`. Pin đánh số vẽ ngay trên phần tử. Sandbox giữ nguyên (origin opaque, không cookie) — mọi thao tác ghi đều từ trang cha. Các ghi chú `Open` được gom vào "Yêu cầu chỉnh sửa" tại cổng POC (xem [delivery-pipeline.md](delivery-pipeline.md#cổng-duyệt-gates--trạng-thái-waitingforhuman)).
  ...
  - **Bố cục hai cột, mỗi cột một câu hỏi.** Cột trái (`.poc-main-col`) là **bản demo + những gì nói về chính bản demo**: khung iframe, rồi ngay dưới nó thẻ "Ghi chú trên POC" (danh sách ghi chú chung, ô nhập, nút đóng vòng "Gửi ghi chú cho đội xử lý", bảng lịch sử ghi chú). Cột phải (`.poc-comments-panel`) chỉ còn phần **đối chiếu yêu cầu**: kịch bản kiểm thử + các vòng đã dựng. Ghi chú ở cột trái vì một mục trong danh sách và một pin vẽ trên demo là cùng một thứ nhìn từ hai phía — để chúng ở hai cột là bắt mắt người dùng nhảy qua lại; và vì trước đây nó nằm SAU toàn bộ danh sách kịch bản, nên POC càng nhiều kịch bản thì đúng hai thao tác KẾT THÚC buổi review càng bị đẩy sâu. Hai hệ quả kỹ thuật: (a) `#pocReviewRoot #pocFrame` trừ thêm chiều cao so với trang khách, nhưng chỉ vừa đủ để **tiêu đề** thẻ ghi chú ló lên trên mép màn hình — không ai cuộn xuống tìm thứ mình không biết là có, mà một khung demo quá lùn thì buộc người xem cuộn BÊN TRONG iframe, tức cuộn một trang nằm trong một trang; (b) click một mục ghi chú sẽ kéo khung demo về tầm mắt (`scrollIntoView`) **chỉ khi** nó đã trôi qua mép trên, nếu không cú nháy pin bên trong iframe là vô hình.
  - **Thẻ chứa khung demo không có padding, và mang icon "mở tab riêng" ở góc trên trái.** `#pocReviewRoot .poc-frame-panel` đặt `padding: 0` (trang khách `#pocShareRoot` giữ nguyên padding vì thẻ đó còn chứa `.poc-toolbar`): bản demo tự mang supergraphic + topbar + sidebar của nó, nên viền trắng của `.card` chỉ là một cái khung thứ hai lồng ngoài cái khung đã có — và nó ăn mất chiều cao của thứ duy nhất trên trang này đáng được chỗ. Đường "mở bản demo ở tab mới" rời command bar xuống làm một icon nổi (`.poc-frame-open`, `<a target="_blank">` nên chuột giữa/Ctrl+click vẫn chạy, không phụ thuộc JS) neo vào góc khung: khác với công tắc ghim, nó không mang trạng thái gì và chỉ tác động lên đúng bản demo đang nằm dưới nó, nên đặt nó chạm vào thứ nó mở thì người dùng khỏi phải đối chiếu một nhãn chữ ở tận thanh lệnh với thứ sẽ được mở.
