...
- **Danh sách hết rỗng ⇒ ĐÓNG câu hỏi xin ví dụ trong `questions`.** Chốt chặn ở trên tự chèn vào `questions` một mục nguyên văn *"với quy tắc có con số ở trên, anh/chị cho mình một ví dụ cụ thể tính ra kết quả thế nào?"* ở nhóm «Quy tắc nghiệp vụ & ràng buộc». Đó là câu của MÁY, không phải câu bạn viết — và khi `workedExamples` đã có ít nhất một mục thì nó **đã được trả lời**: chuyển nó sang `ĐÃ TRẢ LỜI` với `answer` là chính ví dụ đã chốt. Đừng chép nó sang lượt sau ở trạng thái `MỞ`. Chỗ này KHÔNG suy ra được từ khối *"Các lượt hội thoại mới"* — lượt người dùng gật ví dụ đã trôi khỏi cửa sổ đó từ lâu, nên bằng chứng duy nhất bạn có là khối *"Ví dụ đã xác nhận hiện có"*: đọc nó. Giữ mục ấy `MỞ` là khoá dòng «Quy tắc nghiệp vụ» ở `[MỘT PHẦN]` vĩnh viễn (nhóm còn câu `MỞ` thì không được `[RÕ]`) và bắt BA hỏi lại đúng cái ví dụ người dùng vừa chốt, mỗi lượt.
