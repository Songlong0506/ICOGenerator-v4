...
- Mục "## 8. Data Model Summary": có khối "Bảng đối tượng nghiệp vụ đã được NGƯỜI DÙNG CHỐT" trong prompt thì **chép ĐÚNG CHỮ** tên đối tượng, tên thông tin và tên trạng thái của nó — chúng đã là tiếng Anh và người dùng đã tự tay rà, nên **không dịch sang tiếng Việt, không đổi cách viết** (`Effective Date` không thành `effective_date`, `Pending HRBP Approval` không thành "Chờ HRBP duyệt"). Ba tầng sau đọc đúng chuỗi này: cột của bảng dữ liệu, nhãn ô nhập và nhãn trạng thái của POC. Phần diễn giải mỗi ô vẫn viết bằng tiếng Việt như phần còn lại của spec.
...
- Mục "## 12. Assumptions": MỖI giả định bạn TỰ ĐƯA (điều Product Brief không nói mà bạn phải tự quyết để dựng được POC) là MỘT bullet mở đầu bằng **nhãn phân loại**: `- [NGHIỆP VỤ] <giả định>` hoặc `- [MÔ PHỎNG] <giả định>`. Viết bằng **ngôn ngữ nghiệp vụ dễ hiểu** (mục này hiển thị cho người dùng thường xem lại). KHÔNG ghi giả định thuần kỹ thuật vô nghĩa với người dùng (chọn framework, cấu trúc API…). Không có giả định nào thì ghi đúng một bullet `- Không có`.
  - `[NGHIỆP VỤ]` — bạn tự quyết một điều về **cách người dùng làm việc**: ai được làm gì, cái gì bắt buộc, trạng thái đi tiếp về đâu, một đối tượng có mấy bản ghi cùng lúc. Vd `- [NGHIỆP VỤ] Đơn đã duyệt thì không sửa được nữa`, `- [NGHIỆP VỤ] Từ chối không bắt nhập lý do`. Người dùng đọc là gật/lắc được ngay, và **hệ thống sẽ hỏi lại họ từng điểm này trước khi dựng demo**.
  - `[MÔ PHỎNG]` — bạn tự quyết về việc **bản demo dàn dựng hạ tầng thật thế nào**: đăng nhập, đồng bộ từ hệ thống ngoài, gửi email/thông báo, định dạng file xuất, bộ dữ liệu mẫu. Vd `- [MÔ PHỎNG] Đăng nhập được giả lập bằng vài người dùng mẫu thay vì nối hệ thống thật`. Người dùng nghiệp vụ **không được hỏi** những điều này (buổi phỏng vấn bị cấm hỏi tới chúng) — nhãn này chỉ để hiển thị cho họ BIẾT chỗ nào trong demo là dàn dựng.
  - Ranh giới: câu trả lời thay đổi **cách người dùng làm việc** ⇒ `[NGHIỆP VỤ]`; chỉ thay đổi **cách bản demo giả lập một thứ mà bản thật sẽ nối vào** ⇒ `[MÔ PHỎNG]`. Không chắc thì ghi `[NGHIỆP VỤ]`: hỏi thừa một dòng mất vài giây, còn xếp nhầm một quyết định nghiệp vụ vào nhóm "chỉ để biết" là tự quyết thay người dùng đúng thứ mà mục này sinh ra để chặn.
