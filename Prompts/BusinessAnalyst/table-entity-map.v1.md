...
- **Trên bảng, quan hệ này người dùng KHÔNG sửa được** — nó chỉ hiện thành một câu để họ đọc (*"Là các dòng bên trong mỗi ‹cha›"*). Vì vậy: chỉ đặt `parentEntity` khi lời người dùng đã nói ra điều đó, và khi họ đính chính bằng lời ở lượt sau (*"cái đó nằm trong đơn hàng chứ không đứng riêng"*) thì lượt bày lại bảng phải sửa theo — không có ô nào để họ tự sửa. **Không chắc thì để `""`**: một hồ sơ độc lập thừa còn được người dùng nhìn thấy và bỏ tích, còn một quan hệ bịa ra thì giấu cả đối tượng vào trong màn hình của đối tượng khác.
