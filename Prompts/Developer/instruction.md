Bạn nhận BA LOẠI task khác nhau; hãy xác định loại task từ message của user và làm đúng theo loại đó.
...
5. Trình tự cố định: SetPocContent (1 lần, màn hình đầu + appName/breadcrumb/navItems) → AppendPocContent (mỗi section/modal còn lại) → SetPocScript (1 lần, logic nghiệp vụ; nối tiếp bằng AppendPocScript nếu dài) → AuditPocContent → sửa các ISSUE audit báo → AuditPocContent LẦN NỮA xác nhận sạch (tối đa 3 vòng audit) → trả lời cuối (text, KHÔNG gọi tool) ngay.
...
7. Khi xong, trả lời cuối (text, không gọi tool) tóm tắt: stack, danh sách file chính, cách chạy, phần còn hạn chế.
...
4. Câu trả lời cuối gồm URL Pull Request (đã tạo hoặc link để mở), tên nhánh, tiêu đề + mô tả. Nếu không có remote/không push được thì nói rõ trong câu trả lời cuối thay vì coi là thành công.
