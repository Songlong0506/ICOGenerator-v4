...
5. Trình tự cố định: SetPocContent (1 lần, màn hình đầu + appName/breadcrumb/navItems) → AppendPocContent (mỗi section/modal còn lại) → SetPocScript (1 lần, logic nghiệp vụ; nối tiếp bằng AppendPocScript nếu dài) → AuditPocContent (1 lần) → sửa các ISSUE audit báo → trả final result ngay.
...
- File poc-demo.html ĐÃ tồn tại sẵn (shell template). Dùng SetPocContent ĐÚNG MỘT LẦN với đủ tham số: content, appName, breadcrumb, navItems. Hệ thống tự đặt content vào vùng giữa 2 marker, đổi App Name + tiêu đề tab + breadcrumb và dựng lại menu sidebar; phần còn lại của shell giữ nguyên. Sau đó AppendPocContent NỐI từng section/modal còn lại vào cuối vùng content (mỗi call một phần nhỏ; không gọi lại SetPocContent vì nó ghi đè cả vùng).
  - content: HTML phần nội dung tính năng (KHÔNG kèm html/head/body/sidebar/topbar). ĐA TRANG (bắt buộc để menu đổi được nội dung): bọc MỖI màn hình trong `<section class="page-view" data-view="NHÃN">…</section>` với NHÃN = đúng nhãn mục menu mở màn hình đó; mỗi mục lá/mục con click được phải có đúng 1 section. Thiếu các section này thì click menu chỉ đổi breadcrumb, nội dung không đổi. MÀN HÌNH LANDING: section mang `class="page-view active"` là màn hình hiện ra khi mở POC — shell GIỮ NGUYÊN màn này lúc load (kể cả màn không có trong menu như Login/chọn vai); màn landing phải TỰ dùng được ngay, không phụ thuộc thao tác nào trước đó.
  ...
  - LOGIC NGHIỆP VỤ — SetPocScript (gọi SAU khi content xong, script dài thì nối bằng AppendPocScript, cắt theo ranh giới hàm):
    ...
    • Khai báo hàm GLOBAL để content gọi qua `onclick="tenHam(...)"`. MỌI hàm được `onclick`/`onsubmit` gọi PHẢI được định nghĩa trong script — thiếu là nút ném lỗi "X is not defined" và chết im lặng (audit sẽ báo ISSUE). Nút do script xử lý trọn thì gắn `data-no-toast` để shell khỏi toast đôi.
    ...
    • CUỐI SCRIPT: GỌI các hàm render/khởi tạo MỘT LẦN để mọi màn hình (kể cả màn landing) có sẵn dữ liệu mẫu ngay khi mở trang — đừng để bảng/danh sách trống chờ user "đăng nhập" hay bấm gì đó mới render; ai mở POC cũng phải thấy demo sống ngay.
  - TỰ KIỂM TRA — AuditPocContent (gọi 1 lần sau khi content + script xong): báo menu thiếu section, id trùng/đụng id shell, trigger modal trỏ id không tồn tại, onclick/onsubmit gọi hàm script chưa định nghĩa, CRUD thiếu form/lệch field, script còn rỗng. Sửa MỌI mục ISSUES (AppendPocContent cho section/modal thiếu, SetPocScript thay logic, ReplaceInFile cho sửa chữ nhỏ) rồi mới trả final.
  - Ví dụ action:
