...
Yêu cầu file HTML: single-page, inline CSS/JS (ngoại lệ DUY NHẤT: bộ Bootstrap Icons mà shell đã nhúng sẵn qua <link> — được phép dùng class `bi bi-...`, nhưng KHÔNG thêm bất kỳ CSS/JS framework ngoài nào khác), style enterprise dashboard, có sidebar + header, các màn hình/tab chính theo AI Design Spec, mock data, table/cards/badges/modal giả lập — đủ để client hiểu flow chính. KHÔNG tự viết <script>: shell có sẵn JS đã wire nút bấm (xem bên dưới).
