...
- `ModelCallLoggingChatClient` (`DelegatingChatClient`): mỗi lần gọi model → đặt deadline, tính trần
  ...
  đây để khỏi lặp.) **Đây là middleware dùng chung** — `LlmClient` (đường BA) cũng compose nó qua
  `ChatClientBuilder` rồi chạy dưới một `ChatClientAgent` (cùng abstraction MAF với các vai worker, chỉ
  khác là BA không quảng bá tool), nên deadline/token-cap/log/dựng-result không bị viết lặp hai nơi.
...
### 5.9. BA chạy trên ChatClientAgent + structured output (opt-in)
Mọi lời gọi của BA chạy trên một **`ChatClientAgent`** (Microsoft Agent Framework) do `LlmClient` dựng —
cùng abstraction với các vai worker, chỉ khác là BA **không quảng bá tool** (chat thuần trả JSON). Đường
text dùng `ChatClientAgent.RunStreamingAsync` (vẫn stream "đang gõ"); đường structured dùng
`ChatClientAgent.RunAsync<T>` (tự sinh `response_format: json_schema` từ `T` rồi deserialize vào
`AgentResponse<T>.Result`) thay vì chỉ nhắc model trả JSON rồi parse văn xuôi. `ILlmClient.ChatStructuredAsync<T>`
lo việc này; khi structured **không** bật/không khả dụng/JSON không khớp schema thì trả `value = null` để caller
**fallback** về parser tay cũ (`RequirementResponseParser`/`BAChatReplyParser`/`RequirementReadinessParser`) —
