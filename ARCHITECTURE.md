...
- `Tools/Execution`     — **hiện thực**: `ToolPolicyService` (kiểm tra tool có được phép gọi),
  `ToolExecutionLogger`, và `ToolSchemaBuilder` (sinh JSON schema cho tham số từ chữ ký method).
- `Tools/Registry`      — `ToolDiscoveryService` quét các method có `[Description]` rồi đồng bộ vào
  bảng `ToolDefinition`; `DynamicToolInvoker` deserialize JSON args của model vào tham số method
  và invoke qua reflection; `ToolRegistry`/`IToolRegistry` lấy danh sách tool theo agent;
  `ToolRuntimeDescriptor` gói (definition + instance + `MethodInfo`) cho một tool runtime.
...
(thêm class mới vào `ToolDiscoveryService.ToolTypes` nếu cần). Registry/reflection tự sinh schema
...
| Thư mục `Tools/Abstractions` chứa lẫn cả class hiện thực (`ToolPolicyService`, `ToolExecutionLogger`, `ToolSchemaBuilder`) | Tách contract (interface) ở `Abstractions`, chuyển hiện thực sang `Tools/Execution` |
