...
- **`BuildBaContextAsync`** render một "bức tranh tổ chức" gọn (~3–4KB): danh sách department + HoD
  ...
  template `Prompts/BA/organization-context.v2.md` (thay thế bản điền tay v1; comment HTML đầu file bị cắt
...
### 5.17. Vòng phản hồi của stakeholder (chia sẻ project + góp ý Brief + annotate POC)

Trước đây persona User cụt đường ở POC (banner "đội Dev sẽ tiếp nhận") và mỗi project chỉ có một
chủ. Nay có một vòng phản hồi khép kín, ba mảnh ghép đều theo pattern sẵn có:

- **Chia sẻ project (`ProjectMember`)**: chủ project (hoặc người có `ProjectsViewAll`) mời đồng
  nghiệp bằng username (modal "Chia sẻ" ở trang Requirements). Thành viên THẤY project trong
  `Projects/Index` (mở rộng `GetProjectListQuery`: chủ HOẶC thành viên) và góp ý được — KHÔNG
  thay chủ: quyền chat BA/duyệt vẫn theo `AppPermission` như trước. (ProjectId, Username) unique;
  username không FK (nhất quán `CreatedByUsername`).
- **Góp ý Product Brief (`BriefComment`)**: panel bên phải trong modal Requirement. Neo tùy chọn
  vào một ĐOẠN TRÍCH (`AnchorText` — bôi đen trên preview; không dùng offset vì brief sinh lại
  nhiều lần), sống theo PROJECT chứ không FK vào `ProjectDocument`. Resolve bởi tác giả / chủ
  project / `ProjectsViewAll`. Nút "Chèn vào chat" gom các góp ý mở thành MỘT tin nhắn điền vào ô
  chat BA — người dùng duyệt rồi tự gửi (human-in-the-loop, không tự đốt token).
- **Annotate POC (`PocAnnotation`, trang `Projects/PocReview`)**: mockup nhúng trong iframe
  `Mockup?annotate=1` — bản HTML đã TIÊM `<script src="/js/poc-annotate-embed.js">` (static file
  phục vụ trước auth nên iframe sandbox opaque-origin vẫn nạp được; CSP sandbox GIỮ NGUYÊN, script
  chỉ nói chuyện với trang cha qua postMessage + handshake hello/ready chống race). Click phần tử
  → chụp nhãn đọc được + CSS path → ghi nhận xét. Vòng đời `Open → Submitted → Processed`:
  "Gửi đội Dev" (quyền mới **`RequirementsComment`**, seed cho User/TeamDev) chuyển cả lô Open sang
  Submitted + bắn notification `PocFeedbackSubmitted` tới người có `DeliveryAdvance`; đội Dev bấm
  "Yêu cầu agent chỉnh sửa POC" gom mọi mục chưa xử lý thành MỘT feedback có cấu trúc đi qua ĐÚNG
  `RequestStageRevisionUseCase` (không thêm cơ chế revision mới). Chiều hỏng an toàn: không enqueue
  được thì annotation giữ trạng thái cũ — lần sau gom lại, không mất góp ý.
