...
public class WorkspaceTools
{
    ...
    [Description("Set the POC feature UI AND customise the page shell for the generated demo, in ONE call. " +
        ...
        "GROUP THE BULK SCREENS: from THREE screens of the same batch upwards, the catalog screens (names ending in \"Catalog\") go under ONE expandable entry and the report/statistics screens under ONE other — never flat at the top level, or the sidebar buries the real business flow under a run of identical CRUD screens. Label the GROUP in the spec's UI language (\"Danh mục\"/\"Catalogs\", \"Báo cáo\"/\"Reports\"); keep each CHILD label the screen name verbatim so it still matches its section's data-view, and do NOT create a section for the group header. The audit reports a missing grouping as an issue. " +
